...
if (isset($_POST['update'])) {
    ...
    $authorID = $con->updateAuthor($authorFirstName, $authorLastName, $authorBirthYear, $authorNationality, $id);
...
<body>
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      ...
      <div class="container my-5 border border-2 rounded-3 shadow p-4 bg-light">
        <h4 class="mt-3">Edit New Author</h4>
        <!-- Update the form with method and action -->
        <form method="POST" action=""> <!-- Added method="POST" -->
          <div class="mb-3">
        <label for="authorFirstName" class="form-label">First Name</label>
        <input type="text" value="<?php echo $data["author_FN"]?>"class="form-control" id="authorFirstName" name="authorFirstName" required> <!-- Added name attribute -->
      </div>
      <div class="mb-3">
        <label for="authorLastName" class="form-label">Last Name</label>
        <input type="text" value="<?php echo $data["author_LN"]?>" class="form-control" id="authorLastName" name="authorLastName" required> <!-- Added name attribute -->
      </div>
      <div class="mb-3">
        <label for="authorBirthYear" class="form-label">Birth Date</label>
        <input type="date" value="<?php echo $data["author_birthday"]?>" class="form-control" id="authorBirthYear" name="authorBirthYear" max="<?= date('Y-m-d') ?>" required> <!-- Added name attribute -->
      </div>
      <div class="mb-3">
      ...
      <button type="submit" name="update" class="btn btn-primary">Edit Author</button>
    </form>
